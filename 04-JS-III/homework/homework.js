...
function cuentoElementos(arreglo) {
  ...
  let resultado = [];
  for(let i = 0; i < arguments.length; i++){
     if(arguments[i] > 18) resultado.push(arguments[i]);
  }
  return resultado;
}
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  let separarNumero = n.toString().split("");
  if(separarNumero[0] === "9"){
     return true;
  } else {
     return false;
  }
...
function todosIguales(arreglo) {
  ...
  for(let i = 0; i < arreglo.length - 1; i++){
     for(let j = 0; j < arreglo.length - 1; j++){
        if(arreglo[i] !== arreglo[j]) {contador + 1}
     }
  }
...
function mesesDelAño(array) {
  ...
  return newArray.length < 3 ? "No se encontraron los meses pedidos" : newArray;
}
...
function breakStatement(numero) {
  ...
  suma = numero;
  for(let i = 0; i < 10; i++){
     newArray.push(suma + 2);
     suma = suma + 2;
     if(suma === i) {
        return "Se interrumpio la conexion" ;
   }
  }
  return newArray;
}
